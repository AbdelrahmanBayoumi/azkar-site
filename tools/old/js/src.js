...
function getVersionCounter(version) {
	return fetch('https://api.github.com/repos/AbdelrahmanBayoumi/Azkar-App/releases/tags/' + version)
		...
		.then((json) => {
			...
			if (json.assets) {
				Array.from(document.getElementsByClassName('number-of-downloads')).forEach((element, index, array) => {
...
function submit(name, email, message) {
	...
	const queryString =
		'/formResponse?&entry.1748222099=' + name + '&entry.336933390=' + email + '&entry.1510931726=' + message;
	('&submit=SUBMIT');
